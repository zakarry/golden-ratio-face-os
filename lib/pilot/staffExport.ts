...
import type { ParticipantSummary } from './staff';
...
async function fetchCaptures(): Promise<CaptureRow[]> {
  ...
  for (let from = 0; ; from += 500) {
    const { data, error } = await supabase.from('pilot_records')
      .select('participant_token, phase, taken_at, framing, metrics, actionable:target_card->actionableCount, headline:prescription->>headline, tri:karte->triangleAnalysis->>label, strengths:karte->strengths, gr:karte->goldenRatio, blueprint_path')
      .not('participant_token', 'is', null).order('taken_at').range(from, from + 499);
    if (error) throw new Error(error.message);
    rows.push(...(data ?? []).map(r => ({ ...r, has_blueprint: !!r.blueprint_path } as unknown as CaptureRow)));
    if (!data || data.length < 500) break;
  }
  return rows;
}

export async function downloadStaffExcel(list: ParticipantSummary[], origin: string) {
  ...
  [...captures]
    ...
    .forEach(c => {
      const row: Record<string, unknown> = {
        ...
        ok: c.framing?.ok === false ? '×' : '○', warn: (c.framing?.warnings ?? []).join('・'), bp: c.has_blueprint ? 'あり' : '',
